...
def test_cross_track_sync_handles_missing_time_and_frame_columns():
    sync = synchronize_track_times(
        win_prob_df=pd.DataFrame({"Time": [1.0], "WinProb": [52]}),
        shot_df=pd.DataFrame({"xG": [0.3, 0.2]}),
        vaep_df=pd.DataFrame({"VAEP": [0.1]}),
        momentum_series=pd.Series([0.05], index=[0.5]),
    )
