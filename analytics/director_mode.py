...
def _numeric_col(df: pd.DataFrame, column: str, *, default: float = 0.0) -> pd.Series:
    """Return a numeric Series aligned to df.index even when source column is missing."""
    if column in df.columns:
        source = df[column]
    else:
        source = pd.Series([default] * len(df), index=df.index, dtype=float)
...
def _build_event_rows(win_prob_df: pd.DataFrame, shot_df: pd.DataFrame, kickoff_df: pd.DataFrame, vaep_df: pd.DataFrame, save_events_df: pd.DataFrame) -> list[DirectorEvent]:
    ...
    if shot_df is not None and not shot_df.empty:
        ...
        shots["xG"] = _numeric_col(shots, "xG", default=0.0)
    ...
    if save_events_df is not None and not save_events_df.empty:
        ...
        saves["SaveImpact"] = _numeric_col(saves, "SaveImpact", default=0.0)
...
def synchronize_track_times(*, win_prob_df: pd.DataFrame, shot_df: pd.DataFrame, vaep_df: pd.DataFrame, momentum_series: pd.Series) -> pd.DataFrame:
    ...
    if shot_df is not None and not shot_df.empty:
        t = _numeric_col(shot_df, "Time", default=0.0)
        if "Time" not in shot_df.columns:
            t = _numeric_col(shot_df, "Frame", default=0.0) / 30.0
        times = pd.concat([times, t], ignore_index=True)
    if vaep_df is not None and not vaep_df.empty:
        t = _numeric_col(vaep_df, "Time", default=0.0)
        if "Time" not in vaep_df.columns:
            t = _numeric_col(vaep_df, "Frame", default=0.0) / 30.0
        times = pd.concat([times, t], ignore_index=True)
    ...
    if shot_df is not None and not shot_df.empty and "xG" in shot_df.columns:
        s = shot_df.copy()
        s["time"] = _numeric_col(s, "Time", default=0.0)
        if "Time" not in s.columns:
            s["time"] = _numeric_col(s, "Frame", default=0.0) / 30.0
        grid = grid.merge(s[["time", "xG"]].rename(columns={"xG": "xg"}), on="time", how="left")
    if vaep_df is not None and not vaep_df.empty and "VAEP" in vaep_df.columns:
        v = vaep_df.copy()
        v["time"] = _numeric_col(v, "Time", default=0.0)
        if "Time" not in v.columns:
            v["time"] = _numeric_col(v, "Frame", default=0.0) / 30.0
