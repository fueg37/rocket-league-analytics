...
EVENT_TYPE_REMAP = {
    "EventType.SHOT_TAKEN": "shot_taken",
    "EventType.SHOT_ON_TARGET": "shot_on_target",
    "EventType.GOAL": "goal",
    "EventType.SAVE": "save",
    "EventType.BLOCK": "block",
    "EventType.CLEAR": "clear",
    "EventType.CHALLENGE_WIN": "challenge_win",
    "EventType.CHALLENGE_LOSS": "challenge_loss",
    "EventType.PASS_COMPLETED": "pass_completed",
    "EventType.TOUCH": "touch",
    "EventType.BOOST_PICKUP": "boost_pickup",
...
def _migrate_event_v2_to_v3(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "event_type" in out.columns:
        out["event_type"] = out["event_type"].astype(str).replace(EVENT_TYPE_REMAP)
...
def migrate_dataframe(df: pd.DataFrame, table_kind: str) -> pd.DataFrame:
    ...
    if table_kind == "event":
        ...
        elif "event_type" in out.columns:
            out["event_type"] = out["event_type"].astype(str).replace(EVENT_TYPE_REMAP)
